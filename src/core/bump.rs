...
impl CommitAnalysis {
    pub fn summary(&self) -> String {
        ...
        if self.revert_count > 0 {
            parts.push(format!("{} revert", self.revert_count));
        }
        ...
        let recommendation = match commit_result {
            Ok(conv) => {
                if conv.breaking() {
                    ...
                    } else if commit_type == Type::REVERT {
                        // `revert:` is part of the Conventional Commits
                        // spec. Treat it like a fix — undoing a prior
                        // change is a user-visible behaviour change that
                        // warrants at least a patch release. A breaking
                        // revert (`revert!:` or with `BREAKING CHANGE:`
                        // footer) is caught by the `conv.breaking()`
                        // branch above and lifted to major.
                        analysis.revert_count += 1;
                        BumpRecommendation::Patch
                    } else {
            ...
            Err(_) => {
                // The conventional-commit parser doesn't recognise
                // git's auto-generated revert format — `Revert "<subject>"`
                // produced by `git revert`. Treat it as a patch-level
                // revert. Required because `git revert` is the easiest
                // way for a contributor to undo a release-tagged change
                // and the result needs to ship as a new release.
                if is_git_autogenerated_revert(&commit.message) {
                    analysis.revert_count += 1;
                    BumpRecommendation::Patch
                } else {
                    analysis.other_count += 1;
...
/// Recognise `git revert`'s auto-generated subject line. Two shapes:
/// `Revert "<original subject>"` and the multi-revert variant
/// `Revert "<subject>"\n\nThis reverts commit <hash>.`. We only look
/// at the first line because that's what `git` always writes — body
/// text varies (the `--no-verify` recipe just suppresses the body).
fn is_git_autogenerated_revert(message: &str) -> bool {
    let first_line = message.lines().next().unwrap_or("");
    first_line.starts_with("Revert \"") && first_line.ends_with('"')
}
...
mod tests {
    ...
    #[test]
    fn test_conventional_revert_recommends_patch() {
        let commits = vec!["revert: undo a previous change".to_string()];
        let recommendation = recommend_bump_for_commits(&commits).unwrap();
        assert_eq!(recommendation, BumpRecommendation::Patch);
    }

    #[test]
    fn test_conventional_revert_with_scope_recommends_patch() {
        let commits = vec!["revert(home): roll back loading.tsx change".to_string()];
        let recommendation = recommend_bump_for_commits(&commits).unwrap();
        assert_eq!(recommendation, BumpRecommendation::Patch);
    }

    #[test]
    fn test_breaking_revert_recommends_major() {
        // `revert!:` is breaking — undoing a public API surface is a
        // breaking change for downstream consumers.
        let commits = vec!["revert!: roll back v2 API".to_string()];
        let recommendation = recommend_bump_for_commits(&commits).unwrap();
        assert_eq!(recommendation, BumpRecommendation::Major);
    }

    #[test]
    fn test_git_autogenerated_revert_recommends_patch() {
        // Default `git revert` writes `Revert "<subject>"` — not a
        // conventional-commit, but a strong signal something shipped
        // got rolled back.
        let commits = vec![
            "Revert \"feat: add experimental flag\"\n\nThis reverts commit abc123.".to_string(),
        ];
        let recommendation = recommend_bump_for_commits(&commits).unwrap();
        assert_eq!(recommendation, BumpRecommendation::Patch);
    }
    ...
    #[test]
    fn test_git_autogenerated_revert_subject_only() {
        let commits = vec!["Revert \"chore(release): @clikd/landing v0.8.0\"".to_string()];
        let recommendation = recommend_bump_for_commits(&commits).unwrap();
        assert_eq!(recommendation, BumpRecommendation::Patch);
    }

    #[test]
    fn test_revert_keyword_not_a_full_revert_is_ignored() {
        // Free-form prose that just *mentions* "Revert" must not be
        // promoted to a release trigger.
        let commits = vec!["docs: explain Revert behaviour".to_string()];
        let recommendation = recommend_bump_for_commits(&commits).unwrap();
        assert_eq!(recommendation, BumpRecommendation::None);
    }
