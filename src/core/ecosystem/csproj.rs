                ...
                match xml.read_event_into(&mut buf) {
                    Ok(Event::Start(ref e)) => {
                        state = match e.name().0 {
                            ...
                            b"BelafInternalDepVersion" => State::DepReqText,
                            _ => State::Scanning,
                        };
                    }
                    ...
                    Ok(Event::Text(ref t)) => match state {
                        ...
                        State::DepReqText => {
                            let r = atry!(
                                t.decode();
                                ["unable to decode XML text in BelafInternalDepVersion of `{}`", p.display()]
                            );
                            let (guid, reqtext) = a_ok_or!(
                                r.split_once('=');
                                ["malformatted BelafInternalDepVersion `{}` in `{}`", r, p.display()]
                            );
            ...
            for guid in dep_guids.drain(..) {
                let (req, text) = if let Some(text) = dep_reqs.get(&guid) {
                        ...
                        .and_then(|cref| app.repo.resolve_history_ref(&cref, csproj))
                    {
                        ...
                        Err(e) => {
                            warn!(
                                "invalid <BelafInternalDepVersion> entry `{}` in `{}`: {}",
                                text,
                                p.display(),
                                e
                            );
                ...
                if req == DepRequirement::Unavailable {
                    ...
                    if !gave_dep_warning_help {
                        warn!("... you likely need to add <BelafInternalDepVersion>{{guid}}={{req}}</BelafInternalDepVersion> to `{}`", p.display());
                        warn!("... Belaf needs this information to know the oldest compatible version of the dependency");
